...
void blur(
    ...
    vec4 packedA = texture(u_packedTexture, A);
    float radiusA = (packedA.a * 2. - 1.);
    float rA = radiusA * float(u_maxCoCRadius);
    ...
    vec2 direction = vec2(float(horizontal), float(!horizontal));

    // scatter as you gather loop
    for(int delta = -u_maxCoCRadius; delta <= u_maxCoCRadius; ++delta) {

        // get the CoC radius at this tap
        vec2 B = A + direction * (float(delta) * texelSize);
        vec4 packedB = texture(u_packedTexture, B);
        float rB = (packedB.a * 2. - 1.) * float(u_maxCoCRadius);
        ...
        near.a = hasNearFieldCoverage * clamp((rB / float(u_maxCoCRadius)) * 4., 0., 1.);
        near.a *= near.a;
        near.a *= near.a;
...
void main() {
    ...
    for(int delta = -u_maxCoCRadius; delta <= u_maxCoCRadius; ++delta) {
        ...
        // get the CoC radius at this tap
        vec2 B = A + vec2(1., 0) * (float(delta) * texelSize);
        vec4 packedB = texture(u_packedTexture, B);
        float rB = (packedB.a * 2. - 1.) * float(u_maxCoCRadius);
    ...
    outNearColor = nearResult;
    outMidFarColor = midFarResult;
}